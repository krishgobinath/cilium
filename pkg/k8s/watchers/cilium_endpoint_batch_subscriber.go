...
func (cs *cebSubscriber) OnAdd(obj interface{}) {
	ceb, _ := obj.(*cilium_v2a1.CiliumEndpointBatch)
	cs.CEBAdded(ceb)
}

func (cs *cebSubscriber) OnUpdate(oldObj, newObj interface{}) {
	oldCEB, _ := oldObj.(*cilium_v2a1.CiliumEndpointBatch)
	newCEB, _ := newObj.(*cilium_v2a1.CiliumEndpointBatch)
	cs.CEBUpdated(oldCEB, newCEB)
}

func (cs *cebSubscriber) OnDelete(obj interface{}) {
	ceb, _ := obj.(*cilium_v2a1.CiliumEndpointBatch)
	cs.CEBDeleted(ceb)
}

// CEBAdded invoked for newly created CEBs, iterates over coreCEPs
// packed in the CEB, converts coreCEP into types.CEP and calls endpointUpdated only for remoteNode CEPs.
func (cs *cebSubscriber) CEBAdded(ceb *cilium_v2a1.CiliumEndpointBatch) {
...
// CEBUpdated invoked for modified CEBs, it compares old CEB and new CEB objects
...
func (cs *cebSubscriber) CEBUpdated(oldCEB, newCEB *cilium_v2a1.CiliumEndpointBatch) {
...
// CEBDeleted invoked for deleted CEBs, iterates over coreCEPs
// and calls endpointDeleted only for remoteNode CEPs.
func (cs *cebSubscriber) CEBDeleted(ceb *cilium_v2a1.CiliumEndpointBatch) {
	for i, ep := range ceb.Endpoints {
		log.WithFields(logrus.Fields{
			"CEBName": ceb.GetName(),
			"CEPName": ep.Name,
